	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		try {
			...
			if (reqURI.indexOf("/index.xhtml") >= 0 || reqURI.indexOf("/about.xhtml") >= 0
					|| reqURI.indexOf("/log_in.xhtml") >= 0
					|| (ses != null && ses.getAttribute("username") != null
							&& ses.getAttribute("roleName").equals("admin"))
					|| reqURI.indexOf("/public/") >= 0 || reqURI.indexOf("/error/") >= 0
					|| reqURI.contains("javax.faces.resource")) {
				chain.doFilter(request, response);
			} else if (reqURI.indexOf("/index.xhtml") >= 0 || reqURI.indexOf("/about.xhtml") >= 0
					|| reqURI.indexOf("/log_in.xhtml") >= 0
					|| (ses != null && ses.getAttribute("username") != null
							&& ses.getAttribute("roleName").equals("teacher") && reqURI.indexOf("/teacher/") >= 0)
					|| reqURI.indexOf("/public/") >= 0 || reqURI.contains("javax.faces.resource")) {
				chain.doFilter(request, response);
			} else if (reqURI.indexOf("/index.xhtml") >= 0 || reqURI.indexOf("/about.xhtml") >= 0
					|| reqURI.indexOf("/log_in.xhtml") >= 0
					|| (ses != null && ses.getAttribute("username") != null
							&& ses.getAttribute("roleName").equals("student") && reqURI.indexOf("/student/") >= 0)
					|| reqURI.indexOf("/public/") >= 0 || reqURI.contains("javax.faces.resource")) {
				chain.doFilter(request, response);
			} else // user didn't log in but asking for a page that is not
					// allowed so take user to login page
			{
				// System.out.println("Redirecciona a index");
				res.sendRedirect(req.getContextPath() + "/log_in.xhtml"); // Anonymous
																			// user.
																			// Redirect
																			// to
																			// login
